...
pub fn main(event_loop:winit::event_loop::EventLoop<()>) {
	// Only windows requires the window to be present before creating the
	// display. Other platforms don't really need one.
	//
	// XXX if you don't care about running on android or so you can safely
	// remove this condition and always pass the window builder.
		...
		*control_flow = ControlFlow::Wait;
		match event {
			...
			| winit::event::Event::NewEvents(winit::event::StartCause::Init) => {
				...
				// which needs a current context on WGL.
			...
			Event::Suspended => {
				...
				// NativeWindow for a GL Surface can appear and disappear at
				// any moment.
				println!("Android window removed");

				// Destroy the GL Surface and un-current the GL Context before
				// ndk-glue releases the window back to the system.
				...
				match event {
					WindowEvent::Resized(size) => {
						if size.width != 0 && size.height != 0 {
							// Some platforms like EGL require resizing GL
							// surface to update the size
							// Notable platforms here are Wayland and macOS,
							// other don't require it and the function is
							// no-op, but it's wise to resize it for portability
							// reasons.
